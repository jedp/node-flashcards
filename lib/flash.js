...
module.exports = function Flash(user) {
  ...
  this.loadDeck = function(callback) {
    ...
    getDeckFromFile(filename, function(err, deck) {
      ...
      self.redisClient.del('deck:'+self.user+':'+self.deckNumber);

      var multi = self.redisClient.multi();
      deck.forEach(function(pair) {
        if (pair[0] && pair[1]) {
          multi.set('vocab:'+self.deckNumber+':'+pair[0], pair[1]);
          multi.lpush('deck:'+self.user+':'+self.deckNumber, pair[0]);
        }
      });
